   ...
   public static void ShowLoggedMenu(User user)
    {
        while (true)
        {
            Console.Clear();  // Czyszczenie ekranu przed wyświetleniem menu
            Console.WriteLine($"Zalogowano jako: {user.FirstName} {user.LastName} ({user.Role})");

            Console.WriteLine("1. Wyświetl zespół");
            if (user.Role == Role.Principal) Console.WriteLine("2. Dodaj zespół");
            ...
            try
            {
                ...
                Console.ReadLine();
                Console.Clear();  // Czyszczenie ekranu po każdej akcji
            }
            catch (Exception ex)
            {
